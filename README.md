...
Custom placeholders? Build the string yourself from `r.items` — each carries the `range` and `original` value, so you can substitute anything (`"••••"` or `"⟨\(item.label.displayName)⟩"`).
...
See [`LICENSE.md`](LICENSE.md) — Desert Ant Labs Source-Available License v1.0. Free for commercial use up to 100,000 MAU per Model; <licensing@desertant.ai> above that. Third-party data and model attributions are in [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md).
