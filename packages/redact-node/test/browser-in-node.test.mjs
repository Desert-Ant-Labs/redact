...
// loudly and helpfully when someone calls Redact.load() in plain Node. LiteRT.js
...
// process (no DOM), call Redact.load, and assert it rejects with guidance
// pointing at `@desert-ant-labs/redact/native`. This is the Node-observable
...
test("default entry redirects to /native when Redact.load() runs in Node", () => {
  ...
  assert.ok(/@desert-ant-labs\/redact\/native/.test(out),
